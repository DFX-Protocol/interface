...
import { DFX_STATS_API_URL } from "config/backend";
...
export async function getLimitChartPricesFromStats(chainId, symbol, period, limit = 1) {
  ...
  const url = `${DFX_STATS_API_URL}/candles/${symbol}?preferableChainId=${chainId}&period=${period}&limit=${limit}`;
...
export async function getChartPricesFromStats(chainId, symbol, period) {
  ...
  const url = `${DFX_STATS_API_URL}/candles/${symbol}?preferableChainId=${chainId}&period=${period}&from=${from}&preferableSource=fast`;
